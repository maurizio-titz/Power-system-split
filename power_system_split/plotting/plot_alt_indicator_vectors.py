...
def calc_likelihood_failure_alt_indicator(co2_lvl: float, n_nodes: int, save_it: bool = True,
                                          verbose: bool = False):
...
def calc_all_secondary_likelihoods(n_nodes=400, verbose: bool = True):
    ...
    co2_lvl_ls = [float(xx.split("Co2L")[-1].split("-")[0]) 
                  for xx in pypsa_file_ls]
...
def plot_probability_secondary_edge_failures(co2_lvl_list: tuple = (.8, .6, .4, .1),
                                             vlims: tuple=(-5, -3), save_fig: bool = False,
    ...
    if plot_diff_to_orig:
        ...
        fig, [ax, ax_old, ax_diff] = plt.subplots(3, len(co2_lvl_list),
                                          sharex='all', sharey='all', figsize=(16, 6))
    #                       subplot_kw={'projection': proj},
    #                       gridspec_kw={})
    
    else:
        fig, ax = plt.subplots(1, len(co2_lvl_list),
                                          sharex='all', sharey='all', figsize=(32, 8))
    ...
    with open("results/sclopf/split_visualization/" + 
              "edge_likelihoods_secondary_all_" + 
              "co2ls_n400.pickle", 'rb') as fh_old_in:
    ...
    for idx_co2, co2_lvl_r in enumerate(co2_lvl_list):
        ...
        nx.draw_networkx_edges(nx_graph, node_pos, width=2, edge_cmap=cmap,
                               edge_vmin=min(vlims), edge_vmax=max(vlims),
                               edge_color=color_edges, ax=ax_r)
        ...
        old_color_edges = [np.log10(old_like_sec_co2_r[edge_r]) 
                           if old_like_sec_co2_r[edge_r] > 1e-12 else -np.inf 
                           for edge_r in nx_graph.edges()]
        nx.draw_networkx_edges(nx_graph, node_pos, width=2, edge_cmap=cmap,
                               edge_vmin=min(vlims), edge_vmax=max(vlims),
                               edge_color=old_color_edges, ax=ax_r_old)
        
        
        diff_color_edges = [np.log10(abs(old_like_sec_co2_r[edge_r] - like_secondary[edge_r])) 
                           #if np.abs(old_like_sec_co2_r[edge_r] - like_secondary[edge_r]) > 1e-12 else -np.inf 
                           for edge_r in nx_graph.edges()]
        nx.draw_networkx_edges(nx_graph, node_pos, width=2, edge_cmap=cmap_diff,
                               edge_color=diff_color_edges, ax=ax_r_diff,
                               edge_vmin=-6, edge_vmax=-2)
    ...
    offset_label = -.8
    ypos_label = .5
    ax[0].text(offset_label, ypos_label, "Indicator Vector",
               transform=ax[0].transAxes)
    ax_old[0].text(offset_label, ypos_label, "Previous Results",
                   transform=ax_old[0].transAxes)
    ax_diff[0].text(offset_label, ypos_label, "Difference",
                    transform=ax_diff[0].transAxes)
    
    fig.subplots_adjust(left=.15, right=.95)
    
    ## Co2 labels
    for idx_r, ax_r in enumerate(ax_diff):
        ax_r.text(.5, -0.2, f"{co2_lvl_list[idx_r]*100} \%",
                  transform=ax_r.transAxes, horizontalalignment='center')
...
def plot_histogram_number_failed_links(co2_lvl: float, 
                                       save_it: bool = True):
