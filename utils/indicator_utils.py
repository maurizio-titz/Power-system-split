...
def load_indicator_vectors(n_nodes, co2l, indicator_type, path_to_indicator_vectors=path_to_indicator_vectors, mask=None, weights=None):
    ...
    if weights is None and isinstance(co2l, float):
        all_vectors = load_indicator_vectors_unweighted_single_lvl(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask)
        return all_vectors

    elif weights is None and isinstance(co2l, list):
        all_vectors = load_indicator_vectors_unweighted_multiple_lvl(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask)
        return all_vectors
        
    elif weights is not None and isinstance(co2l, float):
        all_vectors, all_weights = load_indicator_vectors_weighted_single_lvl(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask, weights)
...
def load_indicator_vectors_weighted_single_lvl(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask, weights):
    all_weights = []
...
def load_indicator_vectors_unweighted_multiple_lvl(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask):
    all_vectors = []
    for i,co2 in enumerate(co2l):
        file_name = f"indicator_vector_{indicator_type}_Co2l{co2}_n{n_nodes}.pklz"
        with gzip.open(path_to_indicator_vectors + "/" + file_name, "rb") as out:
...
def load_indicator_vectors_unweighted_single_lvl(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask):
    all_vectors = []
    file_name = f"indicator_vector_{indicator_type}_Co2l{co2l}_n{n_nodes}.pklz"
    with gzip.open(path_to_indicator_vectors + "/" + file_name, "rb") as out:
        all_vectors = pickle.load(out)
...
def create_component_indicator_vectors(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask=None):
    ...
    for i,indicator_vector in enumerate(tqdm(indicator_vectors)):
        vals, indices, counts = np.unique(indicator_vector, return_inverse=True, return_counts=True)
        ...
        split_ind_to_component_ind_list.append(np.arange(component_indicators_ind, component_indicators_ind + len(vals)-1))
        component_indicators_ind += len(vals)-1 # -1 because main component is not a split component
            
    file_name = f"indicator_vector_{indicator_type}_components_Co2l{co2l}_n{n_nodes}.pklz"
    with gzip.open(path_to_indicator_vectors + "/" + file_name, "wb") as out:
        pickle.dump((split_ind_to_component_ind_list, np.array(component_indicator_vectors)), out)
        

def calc_is_main_component_indicator_vectors(lshare_vectors, main_component_by="main_comp_max_lshare"):
    ...
    if main_component_by == "main_comp_max_lshare":
        for lshare in lshare_vectors:
            ...
            indicator_vectors.append( np.array(lshare == main_component_lshare, dtype=int))
        ...
        raise ValueError("main_component_by has to be either 'main_comp_max_lshare' or 'main_comp_most_frequent_lshare'")
