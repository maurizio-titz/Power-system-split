...
# def load_indicator_vectors(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask=None):
#     if (isinstance(co2l, list) or isinstance(co2l, np.array)) and not isinstance(mask,list):
#         #throw error if mask is not a list
...
def create_component_indicator_vectors(n_nodes, co2l, indicator_type, path_to_indicator_vectors, mask=None):
    """creates component wise indicator vectors from indicator vectors. Loads indicator vectors from path_to_indicator_vectors and saves component indicator vectors to path_to_indicator_vectors. The main componen is not considered a split component. Accordingly splits that contain n components are projected onto n-1 component indicator vectors. The mapping from the original indicator vector index to component indicator vector index is saved as a array.
...
def calc_is_main_component_indicator_vectors(lshare_vectors, main_component_by="main_comp_max_lshare"):
    ...
    # if not isinstance(lshare_vectors, list):
    #     lshare_vectors = [lshare_vectors]
