...
def cluster_indicator_vectors_combined(
    indicator_vectors,
    indicator_type,
    min_cluster_distance=0.1,
    subset=0.05,
    ...
    print(
        "\n",
        subset_indices.size,
        "components will be used during agglomerative clustering.",
    )
    ...
    n_cluster, subset_c_labels = cluster_indicator_vectors_agglomerative(
        indicator_vectors[subset_indices],
    ...
    with open(
        save_dir
        + f"cluster_labels_subset_{affinity}_minDist{min_cluster_distance}.pklz",
        "wb",
    ) as outfile:
    ...
    for cluster_label in np.unique(subset_c_labels):
        ...
        centroids.append(
            (indicator_vectors[subset_indices][current_indices]).mean(axis=0)
        )
    ...
    rnc = RadiusNeighborsClassifier(
        radius=min_cluster_distance, metric="hamming", outlier_label=-1, n_jobs=n_jobs
    )
...
def inertia_placement(
    ...
    inertia_placement_info = pd.DataFrame(
        columns=["new_var", "added_node_index"], dtype=np.float
    )
    ...
    while var_targ > var_ref * (1 + epsilon):
        ...
        for count, node in enumerate(node_list):
            ...
            comp_props_proposal.loc[:, "inertia_proxy"] = (
                comp_props_new.loc[:, "inertia_proxy"] + new_inertia
            )
            comp_props_proposal.loc[:, "rocof"] = calc_rocof(
                comp_props_proposal.load_imbalance, comp_props_proposal.inertia_proxy
            )

            var_proposals[count] = val_at_risk(
                comp_props_proposal, number_of_simulations, q, method="abs"
            )

        # Select proposal and place inertia at best node
        count_opt = np.argmin(var_proposals)
        comp_props_new.loc[:, "inertia_proxy"] += add_syn_inertia(
            indicator_vectors, count_opt, m
        )
