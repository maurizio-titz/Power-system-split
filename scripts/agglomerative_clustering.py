...
path_to_eval_results= path_to_evaluation_results
# save_path = results_path + 'sclopf/split_visualization/'
save_path = path_to_cascade_results
...
for co2 in co2l_list:
    file_name = f"split_mask_Co2l{co2}_n{n_nodes}_n{n_nodes_split}_l{lost_load_share}.pklz"
    with gzip.open(path_to_indicator_vectors + "/" + file_name, "rb") as infile:
        mask = pickle.load(infile)
    masks.append(mask)

print("loading indicator vectors...")
indicator_vectors = load_indicator_vectors(n_nodes, co2l_list, indicator_type, path_to_indicator_vectors, masks)
...
# indicator_vectors = np.load(path_to_indicator_vectors + f'indicator_vectors_all_n{n_nodes}.npy')
        ...
        n_cluster, cluster_labels = vis.cluster_indicator_vectors_combined(indicator_vectors,
                                                                        ...
                                                                        save_dir=path_to_clustering_results+f"agglom_{indicator_type}/")
