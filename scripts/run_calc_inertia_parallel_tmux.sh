#!/usr/bin/env bash
# Launch one detached tmux session per CO2 level to run calc_inertia_placement.py in parallel

set -euo pipefail

WORKDIR="/srv/data/mtitz/power-system-split"
SCRIPT="scripts/calc_inertia_placement.py"
LOGDIR="$WORKDIR/logs/inertia_placement"

CO2_LEVELS=(0.6 0.5 0.4 0.3 0.2 0.1 0.05 0.0)
SESSION_NAME="inertia_placement"
...
for i in $(seq 0 $((${#CO2_LEVELS[@]} - 1))); do
    ...
    tmux send-keys -t "$SESSION_NAME:$window_idx" "echo 'Started $(date) - CO2=$lvl'" Enter
