...
inertia_by_snapshot_df = pd.DataFrame(inertia_by_snapshot_rows, index=snapshot_index).fillna(0.0)
inertia_by_snapshot_df["total"] = inertia_by_snapshot_df.sum(axis=1)
#%%
inertia_by_snapshot_relative = inertia_by_snapshot_df.div(inertia_by_snapshot_df.total, axis=0)
no_inertia_cols = inertia_by_snapshot_relative.columns[inertia_by_snapshot_relative.sum(axis=0) == 0]
inertia_by_snapshot_relative = inertia_by_snapshot_relative.drop(columns=no_inertia_cols)
